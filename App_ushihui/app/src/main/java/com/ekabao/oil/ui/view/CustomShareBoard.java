...
import com.ekabao.oil.bean.EgeGetRate;
...
//    	Config.dialog = dialog;
        View rootView = LayoutInflater.from(context).inflate(R.layout.popup_share, null);
    ...
    public void onClick(View v) {
		...
		if(flag!=null){
			if(flag.equalsIgnoreCase("zhengchang")){
				image = new UMImage(mActivity, UrlConfig.fenxinaglogo);
			}else if(flag.equalsIgnoreCase("shuangdan")){
				image = new UMImage(mActivity, "https://m.yinongfin.com/images/activity_shangdan.png");
			}else if(flag.startsWith("newyearshare")){
				image = new UMImage(mActivity, "https://m.yinongfin.com/images/luckyMoney.jpg");
			}
			else if(flag.startsWith("toujisong")){
				image = new UMImage(mActivity, "https://m.yinongfin.com/images/app/toujisong_fenxiang.jpg");
			}
			else if(flag.startsWith("app2lottery")){
				image = new UMImage(mActivity, "https://m.yinongfin.com/images/app2lottery.png");
			}
			else if (flag.startsWith("special")){//special
				image = new UMImage(mActivity, "https://m.yinongfin.com/images/ip7.png");
			}
		}else{
...
//				}
    ...
    private void performSharewechat_circle(SHARE_MEDIA platform) {
		...
		if(flag.equalsIgnoreCase("shuangdan")){
			...
			web.setThumb(image);  //缩略图
			web.setDescription("智慧出借，0元消费~ ");//描述
			new ShareAction(mActivity)
					.setPlatform(platform)
					.withMedia(web)
					.withMedia(web)
    ...
    private void performShare(SHARE_MEDIA platform) {
		...
		if(flag!=null){
			if(flag.equalsIgnoreCase("zhengchang")){
				UMWeb web = new UMWeb(url+"?p=" + recommCodes+"&r="+ recommCodes);
				web.setTitle(mActivity.getResources().getString(R.string.triple_title));//标题
				web.setThumb(image);  //缩略图
