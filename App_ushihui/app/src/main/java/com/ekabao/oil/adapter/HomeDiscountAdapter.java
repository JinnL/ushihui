...
public class HomeDiscountAdapter extends BaseQuickAdapter<HomeHostProduct, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, HomeHostProduct item) {
        ...
        ForegroundColorSpan foregroundColorSpan = new ForegroundColorSpan(LocalApplication.getContext().getResources().getColor(R.color.primary));
        StyleSpan ab = new StyleSpan(Typeface.BOLD);
        stringBuilder.setSpan(absoluteSizeSpan, 0, stringBuilder.length() - 2, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        stringBuilder.setSpan(foregroundColorSpan, 0, stringBuilder.length() - 2, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        stringBuilder.setSpan(ab, 0, stringBuilder.length() - 2, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
