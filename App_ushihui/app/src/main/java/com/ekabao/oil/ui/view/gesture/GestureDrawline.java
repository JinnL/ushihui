package com.ekabao.oil.ui.view.gesture;
...
public class GestureDrawline extends View {
	...
	public GestureDrawline(Context context, List<GesturePoint> list,
		...
		paint.setColor(Color.rgb(49,211,138));// 设置默认连线颜色
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (currentPoint != null) {
				currentPoint.setPointState(Constants.SELECTED);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null
				...
				if (betweenPoint != null
					...
					// 设置中间点选中
					betweenPoint.setPointState(Constants.SELECTED);
					// 赋值当前的point;
					currentPoint = pointAt;
				} else {
	...
	final class clearStateRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				p.setPointState(Constants.NORMAL);
			}
			invalidate();
			isDrawEnable = true;
		}
	...
	private void drawErrorPathTip() {
		...
		paint.setColor(Color.rgb(244,87,85));// 设置默认线路颜色
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			pair.first.setPointState(Constants.WRONG);
			pair.second.setPointState(Constants.WRONG);
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
					pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
		}
		invalidate();
