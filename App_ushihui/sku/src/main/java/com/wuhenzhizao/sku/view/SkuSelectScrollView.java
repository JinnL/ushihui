...
public class SkuSelectScrollView extends SkuMaxHeightScrollView implements SkuItemLayout.OnSkuItemSelectListener {
    ...
    private List<String> value = new ArrayList<>(); // 存放第一行的元素
    ...
    public void setSkuList(List<Sku> skuList) {
        ...
        // Log.e("lf", value.size()+"setSkuList" + value.get(0));
    ...
    public void setSelectedSku(Sku sku) {
        ...
        for (SkuAttribute attribute : sku.getAttributes()) {
            ...
            Log.e("lf", "设置选中的sku" + attribute.getKey() + attribute.getValue());
      ...
      /*  SkuAttribute skuAttribute = sku.getAttributes().get(0);
        ...
        selectedAttributeList.add(new SkuAttribute(skuAttribute.getKey(), skuAttribute.getValue()));

        for (int i = 0; i < skuContainerLayout.getChildCount(); i++) {
            SkuItemLayout itemLayout = (SkuItemLayout) skuContainerLayout.getChildAt(i);


            itemLayout.optionItemViewSelectStatus(selectedAttributeList.get(0));

        }*/
    }

    public void setfirstlineStatus() {
        //设置第一行为可点击状态
        SkuItemLayout itemLayout = (SkuItemLayout) skuContainerLayout.getChildAt(0);
        itemLayout.setfirstlineStatus(value);
    }
