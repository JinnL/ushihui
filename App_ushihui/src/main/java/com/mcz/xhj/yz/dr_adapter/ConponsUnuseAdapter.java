...
public class ConponsUnuseAdapter extends BaseAdapter {
    private Context context;
    private List<ConponsBean> lsct;
    private LayoutInflater inflater;
    private String flag;//产品详情选择了优惠券标识id
    private onButtonClickL lis;
    private SpannableString ss;

    public ConponsUnuseAdapter(Context context, List<ConponsBean> lsct, String flag, onButtonClickL lis) {
        super();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder vh = null;
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.item_conpons_unuse1, null);
            vh = new ViewHolder();
        ...
        if(cb.getId().equalsIgnoreCase(flag)){
            vh.image_checked.setVisibility(View.VISIBLE);
        }else {
            vh.image_checked.setVisibility(View.GONE);
        }
        if("".equalsIgnoreCase(flag)||cb.getId().equalsIgnoreCase(flag)){
            if(position==0){
                if(lsct.get(0).getStatus()==3){
                    vh.tv_tab.setVisibility(View.GONE);
                }else{
                    vh.tv_tab.setVisibility(View.VISIBLE);
                }
            }else{
                vh.tv_tab.setVisibility(View.GONE);
            }
        }
//        if(cb.getStatus()==3&&position==0){
//
//        }
        switch (cb.getType()) {
            case 1:// 返现
                vh.ll_conpons_bg.setBackgroundResource(R.mipmap.bg_conpons_yellow);
...
//			vh.bg.setBackgroundResource(R.mipmap.card_bg_red);
                if (cb.getType() == 2) {
                    ...
                    vh.ll_conpons_bg.setBackgroundResource(R.mipmap.bg_conpons_interest);
...
//			vh.tv_money.setText("+"+rate_h);
                ...
                break;
            case 4:// 翻倍券
                vh.ll_conpons_bg.setBackgroundResource(R.mipmap.bg_conpons_double);
