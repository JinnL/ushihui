...
//		}
    ...
    public void pushRegisterId() {
        ...
        logins.put("machineId", imei + "|" + mac);
        logins.put("registrationId", registerId);
        logins.put("appKey", UrlConfig.PUSHAPPKEY);
       // logins.put("version", UrlConfig.version);
        //logins.put("channel", "2");

        OkHttpEngine.create().setHeaders().post(UrlConfig.PUSHREGISTRATIONID, login, new SimpleHttpCallback() {
            @Override
            public void onLogicSuccess(String data) {

               LogUtils.i("--->注册推送pushregistrationid："+data);
...
//							ToastMaker.showShortToast("registerId发送成功");
            ...
            public void onLogicError(int code, String msg) {
                LogUtils.i("--->注册推送pushregistrationid："+msg);
            }

            @Override
            public void onError(IOException e) {
                LogUtils.i("--->注册推送pushregistrationid："+e.toString());
            }
